...
We don't use a VPC in this guide, but it's worth knowing why we don't need one.

Cloud Run, Cloud Storage, and Secret Manager are all inside GCP's network. Cloud Run connects to:
- **Secret Manager** — via GCP's internal API (no firewall needed)
- **Cloud Storage** — via internal GCP network
- **PlanetScale** — over the public internet (but Terraform creates a serverless VPC attachment to avoid this)

For PlanetScale, we use a **Private Service Connect** or **Serverless VPC Access** to connect Cloud Run to PlanetScale without going over the public internet.
